...
function probando(req, res){
	...
	res.status(200).send({
		message: 'Accion de pruebas publications controller'
	});
...
function savePublication(req,res){
	...
	publication.save((err, publicationStored) => {
		if(err) return res.status(500).send({ message: ' Error al gurdar la publcacion'});
...
function getPublication(req,res){
	...
	Publication.findById(publicationId, (err, publication) =>{
		...
		if(err) return res.status(500).send({ message: ' Error al devolver publicaciones'});
		if(!publication) return res.status(404).send({message: ' No existe la publication'});
 ...
 function uploadImage(req,res){
	...
	if(req.files){
		...
		if(file_ext == 'png' || file_ext == 'jpg' || file_ext == 'jpeg' || file_ext == 'gif'){
			//Actualizar doc de publication
			Publication.findOne({'user': req.user.sub, '_id': publicationId}).exec((err, publication) =>{
				console.log('hola '+  publication);
				if(publication){
					Publication.findByIdAndUpdate(publicationId,{file:file_name},{new:true}, (err,publicationUpdated) =>{
					if(err) return res.status(500).send({message:'Error en la peticion'});

					if(!publicationUpdated) return res.status(404).send({message: 'No se ha podido actualiar la publicacion'});
